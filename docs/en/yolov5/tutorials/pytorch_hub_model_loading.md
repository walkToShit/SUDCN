...
model = torch.hub.load("ultralytics/yolov5", "custom", path="path/to/SUDCN.pt")  # local model
model = torch.hub.load("path/to/yolov5", "custom", path="path/to/SUDCN.pt", source="local")  # local repo
```
